...
class TriathlonEventTests: XCTestCase {
	...
	func testRunSportForParticipantAndRaceTimeForParticipantFinishingValueLow() {
		...
		XCTAssertNotNil(raceTime, "TriathlonEvent race time for registered participant after call to simulate(_:for:) with a random value of 0.05 was nil")
		if let someRaceTime = raceTime {
			XCTAssertGreaterThan(someRaceTime, 0, "TriathlonEvent race time for registered participant after call to simulate(_:for:) with a random value of 0.05 was not greater than 0")
		}
	}
	...
	func testRunSportForParticipantAndRaceTimeForParticipantNoFavoriteNonFinishingValueHigh() {
		...
		let testParticipant = TestParticipant(name: "Foo")
		triathlonEvent.register(testParticipant)
		triathlonEvent.simulate(Sport.swimming, for: testParticipant, randomValue: 0.04)
		let raceTime = triathlonEvent.raceTime(for: testParticipant)
		XCTAssertNil(raceTime, "TriathlonEvent race time for registered participant after call to simulate(_:for:) with a random value of 0.04 was not nil")
	}
